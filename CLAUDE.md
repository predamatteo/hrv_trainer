...
Riverpod + go_router. **4-tab bottom-nav shell** via `StatefulShellRoute.indexedStack` in `core/router/app_router.dart`: branches Home (`/`), Sessione (`/sessione` — practice hub in `features/sessione/`), Storico (`/history`), Profilo (`/settings`). `/hrv` (Andamento HRV) and `/readiness` are children of the Home branch; `/history/session/:id` is a child of Storico. The immersive flows (`/training`, `/pacer`, `/assessment`, `/readiness/checkin`) are **top-level routes with `parentNavigatorKey = rootNavigatorKey`** so they cover the bottom nav (modal in spirit — wakelock + `PopScope`). The shell widget is `core/router/scaffold_with_nav_bar.dart`. Each `features/<name>/` holds its screen + `state/` controller(s) + `widgets/`.
