...
`lib/shared/storage/database.dart` — sqflite, **currently version 3**. Tables: `sessions` (metrics/pattern/morning-meta as JSON columns), `assessments`, `rr_samples` (FK cascade). Migrations are additive `ALTER TABLE ... ADD COLUMN` only — preserve that pattern (v2 added `tag`, v3 added `morning_meta_json`). `session_repository.dart` is the data API, including `exportAll()`/`importAll()` (JSON backup, schema v1, dedup on `startedAt`) for the manual Drive/email backup feature.

### Phone app structure

Riverpod + go_router. Routes in `core/router/app_router.dart` (`/`, `/pacer`, `/assessment`, `/training`, `/history`, `/hrv`, `/readiness`, `/readiness/checkin`, `/settings`). Each `features/<name>/` holds its screen + `state/` controller(s) + `widgets/`. Theme: petrol + moss-green M3 in `core/theme/`. Local reminder notifications via `shared/notifications/`.
...
- `test/widget_test.dart` "App avvia senza crash" can fail on a pending 2s timer from `RemoteSessionPersister` — this is pre-existing, not a regression you introduced.
